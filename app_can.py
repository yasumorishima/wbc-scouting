"""WBC 2026 Canada Batter Scouting Dashboard."""
...
from players_can_batters import CAN_BATTERS, PLAYER_BY_NAME
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "can_statcast.csv"
...
def main():
    st.set_page_config(
        page_title="Canada Scouting — WBC 2026",
    ...
    _name_ja_map = {p["name"]: p.get("name_ja", "") for p in CAN_BATTERS}
    ...
    player_names = [t["team_overview"]] + [p["name"] for p in CAN_BATTERS]
    ...
    season_label = f"{season} Season" if lang == "EN" else f"{season}年シーズン"
    st.header(f"\U0001F1E8\U0001F1E6 {_display_name(player['name'])} — {season_label}")

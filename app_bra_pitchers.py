"""WBC 2026 Brazil Pitching Scouting Dashboard."""
...
from players_bra_pitchers import BRA_PITCHERS, PITCHER_BY_NAME
...
TEXTS = {
    "EN": {
        "title": "Brazil Pitching Scouting Report",
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "bra_pitchers_statcast.csv"
...
def main():
    ...
    _name_ja_map = {p["name"]: p.get("name_ja", "") for p in BRA_PITCHERS}
    ...
    pitcher_names = [t["team_overview"]] + [p["name"] for p in BRA_PITCHERS]
    ...
    season_label = f"{season} Season" if lang == "EN" else f"{season}年シーズン"
    st.header(f"\U0001F1E7\U0001F1F7 {_display_name(pitcher['name'])} — {season_label}")

"""WBC 2026 Israel Batter Scouting Dashboard."""
...
from players_isr_batters import ISR_BATTERS, PLAYER_BY_NAME
...
TEXTS = {
    "EN": {
        "title": "Israel Batter Scouting Report",
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "isr_statcast.csv"
...
def main():
    st.set_page_config(
        page_title="Israel Scouting — WBC 2026",
        page_icon="\U0001F1EE\U0001F1F1",
        layout="wide",
    ...
    st.sidebar.markdown(f"# \U0001F1EE\U0001F1F1 {t['title']}")
    st.sidebar.caption(t["subtitle"])

    _name_ja_map = {p["name"]: p.get("name_ja", "") for p in ISR_BATTERS}
    ...
    player_names = [t["team_overview"]] + [p["name"] for p in ISR_BATTERS]
    ...
    season_label = f"{season} Season" if lang == "EN" else f"{season}年シーズン"
    st.header(f"\U0001F1EE\U0001F1F1 {_display_name(player['name'])} — {season_label}")

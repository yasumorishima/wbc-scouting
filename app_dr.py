...
TEXTS = {
    "EN": {
        ...
        "strength_note": (
            "The Dominican Republic boasts one of the most powerful lineups in the tournament, "
            "led by Soto, Guerrero Jr., Tatís Jr., and Machado. "
            "The lineup combines elite power with strong on-base skills (Soto, Wells). "
            "Key vulnerabilities: some aggressive hitters carry elevated K-rates, "
            "and certain batters show platoon splits that can be exploited "
            "with opposite-hand pitching matchups."
        ),
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "dr_statcast.csv"
...
def main():
    ...
    _name_ja_map = {p["name"]: p.get("name_ja", "") for p in DR_BATTERS}
    ...
    player_names = [t["team_overview"]] + [p["name"] for p in DR_BATTERS]
    ...
    if selected == t["team_overview"]:
        ...
        st.header(f"\U0001F1E9\U0001F1F4 {t['team_overview']} — {season_label}")
    ...
    season_label = f"{season} Season" if lang == "EN" else f"{season}年シーズン"
    st.header(f"\U0001F1E9\U0001F1F4 {_display_name(player['name'])} — {season_label}")

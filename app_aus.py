"""WBC 2026 Australia Batter Scouting Dashboard."""
...
from players_aus_batters import AUS_BATTERS, PLAYER_BY_NAME
...
TEXTS = {
    "EN": {
        "title": "Australia Batter Scouting Report",
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "aus_statcast.csv"
...
def main():
    st.set_page_config(
        page_title="Australia Scouting — WBC 2026",
    ...
    _name_ja_map = {p["name"]: p.get("name_ja", "") for p in AUS_BATTERS}
    ...
    player_names = [t["team_overview"]] + [p["name"] for p in AUS_BATTERS]
    ...
    season_label = f"{season} Season" if lang == "EN" else f"{season}年シーズン"
    st.header(f"\U0001F1E6\U0001F1FA {_display_name(player['name'])} — {season_label}")

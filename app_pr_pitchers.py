"""WBC 2026 Puerto Rico Pitching Scouting Dashboard."""
...
TEXTS = {
    "EN": {
        "title": "Puerto Rico Pitching Scouting Report",
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "pr_pitchers_statcast.csv"
...
def main():
    st.set_page_config(
        page_title="Puerto Rico Pitching \u2014 WBC 2026",
    ...
    _name_ja_map = {p["name"]: p.get("name_ja", "") for p in PR_PITCHERS}
    ...
    pitcher_names = [t["team_overview"]] + [p["name"] for p in PR_PITCHERS]
    ...
    season_label = f"{season} Season" if lang == "EN" else f"{season}年シーズン"
    st.header(f"\U0001F1F5\U0001F1F7 {_display_name(pitcher['name'])} — {season_label}")

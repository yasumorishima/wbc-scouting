"""WBC 2026 Brazil Batter Scouting Dashboard."""
...
from players_bra_batters import BRA_BATTERS, PLAYER_BY_NAME
...
TEXTS = {
    "EN": {
        "title": "Brazil Batter Scouting Report",
        ...
        "strength_note": (
            "Brazil is an emerging baseball nation with a growing presence on the international stage. The lineup features players from the domestic league and professional organizations, with improving talent depth as the Brazilian baseball program continues to develop."
        ),
    ...
    "JA": {
        ...
        "strength_note": (
            "ブラジルは国際舞台での存在感を高めつつある新興野球国である。\n\n国内リーグとプロ組織の選手で構成されており、ブラジルの野球プログラムの成長とともに選手の質も向上していると考えられる。"
        ),
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "bra_statcast.csv"
...
def main():
    st.set_page_config(
        page_title="Brazil Scouting — WBC 2026",
    ...
    st.sidebar.markdown(f"# \U0001F1E7\U0001F1F7 {t['title']}")
    ...
    if df_all is None:
        st.warning(
            "No Statcast data available for Brazil batters yet. "
            "Data will appear here once these players record MLB appearances."
            if lang == "EN" else
            "ブラジル打者のStatcastデータはまだありません。"
            "MLB出場後にデータが表示されます。"
        )
        st.stop()
    ...
    if selected == t["team_overview"]:
        ...
        st.header(f"\U0001F1E7\U0001F1F7 {t['team_overview']} — {season_label}")
    ...
    season_label = f"{season} Season" if lang == "EN" else f"{season}年シーズン"
    st.header(f"\U0001F1E7\U0001F1F7 {_display_name(player['name'])} — {season_label}")

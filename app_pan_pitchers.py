"""WBC 2026 Panama Pitching Scouting Dashboard."""
...
from players_pan_pitchers import PAN_PITCHERS, PITCHER_BY_NAME
...
TEXTS = {
    "EN": {
        "title": "Panama Pitching Scouting Report",
        ...
        "strength_note": (
            "Panama's pitching staff includes arms with MLB and MiLB experience, capable of generating swings and misses with power stuff. The bullpen may be a key strength for Panama's tournament run."
        ),
        "overview_guide": (
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "pan_pitchers_statcast.csv"
...
def main():
    ...
    pitcher_names = [t["team_overview"]] + [p["name"] for p in PAN_PITCHERS]
    ...
    season_label = f"{season} Season" if lang == "EN" else f"{season}年シーズン"
    st.header(f"\U0001F1F5\U0001F1E6 {_display_name(pitcher['name'])} — {season_label}")

"""Fetch Statcast data for Puerto Rico WBC batters/pitchers and save to CSV.

Usage:
    python fetch_data_pr.py                    # batters, 2024 + 2025
    python fetch_data_pr.py --pitchers         # pitchers, 2024 + 2025
    python fetch_data_pr.py --pitchers --year 2024
"""
...
from players_pr_batters import PR_BATTERS
from players_pr_pitchers import PR_PITCHERS
...
OUT_PATH_BATTER = "data/pr_statcast.csv"
OUT_PATH_PITCHER = "data/pr_pitchers_statcast.csv"


def fetch_batters(years: list[int]) -> pd.DataFrame:
    frames = []
    total = len(PR_BATTERS) * len(years)
    idx = 0
    for year in years:
...
def fetch_pitchers(years: list[int]) -> pd.DataFrame:
    frames = []
    total = len(PR_PITCHERS) * len(years)
    idx = 0
    for year in years:
        start, end = SEASONS[year]
        for p in PR_PITCHERS:
            idx += 1
            print(f"[{idx}/{total}] {p['name']} ({year}) ...", end=" ", flush=True)
            try:
...
def main():
    ...
    if args.pitchers:
        print(f"Fetching Statcast data for {len(PR_PITCHERS)} PR pitchers, seasons {years}")
        combined = fetch_pitchers(years)
        combined.to_csv(OUT_PATH_PITCHER, index=False)
        print(f"\nSaved {len(combined)} rows to {OUT_PATH_PITCHER}")
    else:
        print(f"Fetching Statcast data for {len(PR_BATTERS)} PR batters, seasons {years}")

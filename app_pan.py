"""WBC 2026 Panama Batter Scouting Dashboard."""
...
from players_pan_batters import PAN_BATTERS, PLAYER_BY_NAME
...
TEXTS = {
    "EN": {
        "title": "Panama Batter Scouting Report",
        ...
        "strength_note": (
            "Panama has a rich baseball history and features a lineup built around MLB-experienced players with power and speed. The team's offensive profile may center on run production through extra-base hits and aggressive baserunning."
        ),
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "pan_statcast.csv"
...
def main():
    st.set_page_config(
        page_title="Panama Scouting — WBC 2026",
    ...
    _name_ja_map = {p["name"]: p.get("name_ja", "") for p in PAN_BATTERS}
    ...
    player_names = [t["team_overview"]] + [p["name"] for p in PAN_BATTERS]
    ...
    if selected == t["team_overview"]:
        ...
        st.header(f"\U0001F1F5\U0001F1E6 {t['team_overview']} — {season_label}")

        st.info(t["overview_guide"])

        rows = []
        player_stats_list = []
        for p in PAN_BATTERS:
    ...
    season_label = f"{season} Season" if lang == "EN" else f"{season}年シーズン"
    st.header(f"\U0001F1F5\U0001F1E6 {_display_name(player['name'])} — {season_label}")

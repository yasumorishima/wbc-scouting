"""WBC 2026 Chinese Taipei Batter Scouting Dashboard."""
...
from players_twn_batters import TWN_BATTERS, PLAYER_BY_NAME
...
TEXTS = {
    "EN": {
        "title": "Chinese Taipei Batter Scouting Report",
        ...
        "strength_note": (
            "Chinese Taipei features a lineup with strong contact-oriented hitters, drawing on players from NPB, CPBL, and MLB organizations. The team emphasizes plate discipline and situational hitting, and may pose a significant challenge through consistent contact and run manufacturing."
        ),
    ...
    "JA": {
        "title": "チャイニーズタイペイ 打者スカウティングレポート",
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "twn_statcast.csv"
...
def main():
    st.set_page_config(
        page_title="Chinese Taipei Scouting — WBC 2026",
        page_icon="\U0001F1F9\U0001F1FC",
        layout="wide",
    ...
    st.sidebar.markdown(f"# \U0001F1F9\U0001F1FC {t['title']}")
    st.sidebar.caption(t["subtitle"])

    _name_ja_map = {p["name"]: p.get("name_ja", "") for p in TWN_BATTERS}
    ...
    player_names = [t["team_overview"]] + [p["name"] for p in TWN_BATTERS]
    ...
    if selected == t["team_overview"]:
        ...
        st.header(f"\U0001F1F9\U0001F1FC {t['team_overview']} — {season_label}")

        st.info(t["overview_guide"])

        rows = []
        player_stats_list = []
        for p in TWN_BATTERS:
    ...
    season_label = f"{season} Season" if lang == "EN" else f"{season}年シーズン"
    st.header(f"\U0001F1F9\U0001F1FC {_display_name(player['name'])} — {season_label}")

"""WBC 2026 Canada Pitching Scouting Dashboard."""
...
from players_can_pitchers import CAN_PITCHERS, PITCHER_BY_NAME
...
TEXTS = {
    "EN": {
        "title": "Canada Pitching Scouting Report",
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "can_pitchers_statcast.csv"
...
def main():
    ...
    st.sidebar.markdown(f"# \U0001F1E8\U0001F1E6 {t['title']}")
    st.sidebar.caption(t["subtitle"])

    _name_ja_map = {p["name"]: p.get("name_ja", "") for p in CAN_PITCHERS}
    ...
    pitcher_names = [t["team_overview"]] + [p["name"] for p in CAN_PITCHERS]
    ...
    if selected == t["team_overview"]:
        ...
        st.header(f"\U0001F1E8\U0001F1E6 {t['team_overview']} — {season_label}")

        st.info(t["overview_guide"])

        rows = []
        pitcher_stats_list = []
        for p in CAN_PITCHERS:
    ...
    season_label = f"{season} Season" if lang == "EN" else f"{season}年シーズン"
    st.header(f"\U0001F1E8\U0001F1E6 {_display_name(pitcher['name'])} — {season_label}")

"""WBC 2026 Japan Pitching Scouting Dashboard."""
...
from players_japan_pitchers import JAPAN_PITCHERS, PITCHER_BY_NAME
...
TEXTS = {
    "EN": {
        "title": "Japan Pitching Scouting Report",
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "japan_pitchers_statcast.csv"
...
def main():
    ...
    _name_ja_map = {p["name"]: p.get("name_ja", "") for p in JAPAN_PITCHERS}
    ...
    pitcher_names = [t["team_overview"]] + [p["name"] for p in JAPAN_PITCHERS]
    ...
    st.header(f"\U0001F1EF\U0001F1F5 {_display_name(pitcher['name'])}")

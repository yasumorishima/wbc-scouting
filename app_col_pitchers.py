"""WBC 2026 Colombia Pitching Scouting Dashboard."""
...
from players_col_pitchers import COL_PITCHERS, PITCHER_BY_NAME
...
TEXTS = {
    "EN": {
        "title": "Colombia Pitching Scouting Report",
        ...
        "strength_note": (
            "Colombia's pitching staff may include several active MLB arms with the ability to miss bats and generate weak contact. The overall depth of the pitching staff is a potential strength."
        ),
        "overview_guide": (
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "col_pitchers_statcast.csv"
...
def main():
    st.set_page_config(
        page_title="Colombia Pitching \u2014 WBC 2026",
    ...
    season_label = f"{season} Season" if lang == "EN" else f"{season}年シーズン"
    st.header(f"\U0001F1E8\U0001F1F4 {_display_name(pitcher['name'])} — {season_label}")

"""Fetch Statcast data for Dominican Republic WBC batters/pitchers and save to CSV.

Usage:
    python fetch_data_dr.py                    # batters, 2024 + 2025
    python fetch_data_dr.py --pitchers         # pitchers, 2024 + 2025
    python fetch_data_dr.py --pitchers --year 2024
"""
...
from players_dr_batters import DR_BATTERS
from players_dr_pitchers import DR_PITCHERS
...
OUT_PATH_BATTER = "data/dr_statcast.csv"
OUT_PATH_PITCHER = "data/dr_pitchers_statcast.csv"


def fetch_batters(years: list[int]) -> pd.DataFrame:
    frames = []
    total = len(DR_BATTERS) * len(years)
    idx = 0
    for year in years:
...
def fetch_pitchers(years: list[int]) -> pd.DataFrame:
    frames = []
    total = len(DR_PITCHERS) * len(years)
    idx = 0
    for year in years:
        start, end = SEASONS[year]
...
def main():
    ...
    if args.pitchers:
        print(f"Fetching Statcast data for {len(DR_PITCHERS)} DR pitchers, seasons {years}")
        combined = fetch_pitchers(years)
        combined.to_csv(OUT_PATH_PITCHER, index=False)
        print(f"\nSaved {len(combined)} rows to {OUT_PATH_PITCHER}")
    else:
        print(f"Fetching Statcast data for {len(DR_BATTERS)} DR batters, seasons {years}")

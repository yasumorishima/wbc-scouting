"""WBC 2026 Italy Pitching Scouting Dashboard."""
...
from players_ita_pitchers import ITA_PITCHERS, PITCHER_BY_NAME
...
TEXTS = {
    "EN": {
        "title": "Italy Pitching Scouting Report",
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "ita_pitchers_statcast.csv"
...
def main():
    ...
    _name_ja_map = {p["name"]: p.get("name_ja", "") for p in ITA_PITCHERS}
    ...
    pitcher_names = [t["team_overview"]] + [p["name"] for p in ITA_PITCHERS]
    ...
    season_label = f"{season} Season" if lang == "EN" else f"{season}年シーズン"
    st.header(f"\U0001F1EE\U0001F1F9 {_display_name(pitcher['name'])} — {season_label}")

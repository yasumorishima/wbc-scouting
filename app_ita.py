"""WBC 2026 Italy Batter Scouting Dashboard."""
...
TEXTS = {
    "EN": {
        "title": "Italy Batter Scouting Report",
        ...
        "strength_note": (
            "Italy has steadily improved its talent pool by drawing on players of Italian descent competing in MLB and MiLB. The lineup features versatile hitters with solid contact skills, and key contributors may include players with notable MLB experience."
        ),
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "ita_statcast.csv"
...
def main():
    st.set_page_config(
        page_title="Italy Scouting — WBC 2026",
    ...
    _name_ja_map = {p["name"]: p.get("name_ja", "") for p in ITA_BATTERS}
    ...
    player_names = [t["team_overview"]] + [p["name"] for p in ITA_BATTERS]
    ...
    if selected == t["team_overview"]:
        ...
        st.header(f"\U0001F1EE\U0001F1F9 {t['team_overview']} — {season_label}")

        st.info(t["overview_guide"])

        rows = []
        player_stats_list = []
        for p in ITA_BATTERS:
    ...
    season_label = f"{season} Season" if lang == "EN" else f"{season}年シーズン"
    st.header(f"\U0001F1EE\U0001F1F9 {_display_name(player['name'])} — {season_label}")

"""Fetch Statcast data for Venezuela WBC batters/pitchers and save to CSV.

Usage:
    python fetch_data.py                    # batters, 2024 + 2025
    python fetch_data.py --pitchers         # pitchers, 2024 + 2025
    python fetch_data.py --pitchers --year 2024
"""
...
from players import VENEZUELA_BATTERS, VENEZUELA_PITCHERS
...
OUT_PATH_BATTER = "data/venezuela_statcast.csv"
OUT_PATH_PITCHER = "data/venezuela_pitchers_statcast.csv"


def fetch_batters(years: list[int]) -> pd.DataFrame:
    frames = []
    total = len(VENEZUELA_BATTERS) * len(years)
    idx = 0
    for year in years:
...
def fetch_pitchers(years: list[int]) -> pd.DataFrame:
    frames = []
    total = len(VENEZUELA_PITCHERS) * len(years)
    idx = 0
    for year in years:
        start, end = SEASONS[year]
...
def main():
    ...
    if args.pitchers:
        print(f"Fetching Statcast data for {len(VENEZUELA_PITCHERS)} pitchers, seasons {years}")
        combined = fetch_pitchers(years)
        combined.to_csv(OUT_PATH_PITCHER, index=False)
        print(f"\nSaved {len(combined)} rows to {OUT_PATH_PITCHER}")
    else:
        print(f"Fetching Statcast data for {len(VENEZUELA_BATTERS)} batters, seasons {years}")

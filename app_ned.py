"""WBC 2026 Netherlands Batter Scouting Dashboard."""
...
from players_ned_batters import NED_BATTERS, PLAYER_BY_NAME
...
TEXTS = {
    "EN": {
        "title": "Netherlands Batter Scouting Report",
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "ned_statcast.csv"
...
def main():
    st.set_page_config(
        page_title="Netherlands Scouting — WBC 2026",
    ...
    _name_ja_map = {p["name"]: p.get("name_ja", "") for p in NED_BATTERS}
    ...
    player_names = [t["team_overview"]] + [p["name"] for p in NED_BATTERS]
    ...
    if selected == t["team_overview"]:
        st.header(f"\U0001F1F3\U0001F1F1 {t['team_overview']}")
    ...
    st.header(f"\U0001F1F3\U0001F1F1 {_display_name(player['name'])}")

"""WBC 2026 Mexico Batter Scouting Dashboard."""
...
from players_mex_batters import MEX_BATTERS, PLAYER_BY_NAME
...
TEXTS = {
    "EN": {
        "title": "Mexico Batter Scouting Report",
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "mex_statcast.csv"
...
def main():
    st.set_page_config(
        page_title="Mexico Scouting — WBC 2026",
    ...
    _name_ja_map = {p["name"]: p.get("name_ja", "") for p in MEX_BATTERS}
    ...
    player_names = [t["team_overview"]] + [p["name"] for p in MEX_BATTERS]
    ...
    season_label = f"{season} Season" if lang == "EN" else f"{season}年シーズン"
    st.header(f"\U0001F1F2\U0001F1FD {_display_name(player['name'])} — {season_label}")

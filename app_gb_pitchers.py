"""WBC 2026 Great Britain Pitching Scouting Dashboard."""
...
TEXTS = {
    "EN": {
        "title": "Great Britain Pitching Scouting Report",
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "gb_pitchers_statcast.csv"
...
def main():
    st.set_page_config(
        page_title="Great Britain Pitching \u2014 WBC 2026",
    ...
    _name_ja_map = {p["name"]: p.get("name_ja", "") for p in GB_PITCHERS}
    ...
    pitcher_names = [t["team_overview"]] + [p["name"] for p in GB_PITCHERS]
    ...
    season_label = f"{season} Season" if lang == "EN" else f"{season}年シーズン"
    st.header(f"\U0001F1EC\U0001F1E7 {_display_name(pitcher['name'])} — {season_label}")

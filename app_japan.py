"""WBC 2026 Japan Batter Scouting Dashboard."""
...
from players_japan_batters import JAPAN_BATTERS, PLAYER_BY_NAME
...
TEXTS = {
    "EN": {
        ...
        "strength_note": (
            "Japan is widely regarded as one of the strongest teams in the tournament, featuring a balanced lineup with elite contact hitters and disciplined plate approach. Key batters include Shohei Ohtani and Munetaka Murakami, both capable of elite power. The lineup's depth and high on-base skills may pose significant challenges for opposing pitchers."
        ),
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "japan_statcast.csv"
...
def main():
    st.set_page_config(
        page_title="Japan Scouting — WBC 2026",
    ...
    st.sidebar.markdown(f"# \U0001F1EF\U0001F1F5 {t['title']}")
    st.sidebar.caption(t["subtitle"])

    _name_ja_map = {p["name"]: p.get("name_ja", "") for p in JAPAN_BATTERS}
    ...
    player_names = [t["team_overview"]] + [p["name"] for p in JAPAN_BATTERS]
    ...
    if selected == t["team_overview"]:
        st.header(f"\U0001F1EF\U0001F1F5 {t['team_overview']}")
    ...
    st.header(f"\U0001F1EF\U0001F1F5 {_display_name(player['name'])}")

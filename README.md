...
> - Nicaragua / Chinese Taipei / Australia: 投手の Statcast データが少量（0〜7行）のため投手ダッシュボードなし
...
- **📋 ゲームプラン（イニング別攻略）** — 1〜3回（先発1巡目）、4〜5回（2巡目）、6回以降（ブルペン戦）の3フェーズに分けて打撃・投球戦略を解説。先発投手の球種使用率、各リリーフのK%/Whiff%/Chase%/球速、打者ごとのAVG/OPS/K%/BB%を全てStatcastデータから動的生成
- **チーム弱点分析** — 三振率の高い打者、積極的な打者（低BB%）、左右差の大きい打者をデータで自動抽出し、MLB平均との比較で「なぜ弱点なのか」を提示

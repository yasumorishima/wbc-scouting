"""WBC 2026 South Korea Pitching Scouting Dashboard."""
...
TEXTS = {
    "EN": {
        "title": "South Korea Pitching Scouting Report",
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "kor_pitchers_statcast.csv"
...
def main():
    ...
    _name_ja_map = {p["name"]: p.get("name_ja", "") for p in KOR_PITCHERS}
    ...
    pitcher_names = [t["team_overview"]] + [p["name"] for p in KOR_PITCHERS]
    ...
    st.header(f"\U0001F1F0\U0001F1F7 {_display_name(pitcher['name'])}")

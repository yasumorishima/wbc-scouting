"""WBC 2026 Mexico Pitching Scouting Dashboard."""
...
from players_mex_pitchers import MEX_PITCHERS, PITCHER_BY_NAME
...
TEXTS = {
    "EN": {
        "title": "Mexico Pitching Scouting Report",
        ...
        "strength_note": (
            "Mexico's pitching staff includes several MLB-level arms with the ability to generate swings and misses across multiple pitch types. The rotation and bullpen depth may provide strong coverage throughout the tournament."
        ),
        "overview_guide": (
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "mex_pitchers_statcast.csv"
...
def main():
    st.set_page_config(
        page_title="Mexico Pitching \u2014 WBC 2026",
    ...
    st.sidebar.markdown(f"# \U0001F1F2\U0001F1FD {t['title']}")
    st.sidebar.caption(t["subtitle"])

    _name_ja_map = {p["name"]: p.get("name_ja", "") for p in MEX_PITCHERS}
    ...
    pitcher_names = [t["team_overview"]] + [p["name"] for p in MEX_PITCHERS]
    ...
    if selected == t["team_overview"]:
        ...
        st.header(f"\U0001F1F2\U0001F1FD {t['team_overview']} — {season_label}")
    ...
    season_label = f"{season} Season" if lang == "EN" else f"{season}年シーズン"
    st.header(f"\U0001F1F2\U0001F1FD {_display_name(pitcher['name'])} — {season_label}")

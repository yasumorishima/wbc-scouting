"""WBC 2026 South Korea Batter Scouting Dashboard."""
...
from players_kor_batters import KOR_BATTERS, PLAYER_BY_NAME
...
TEXTS = {
    "EN": {
        "title": "South Korea Scouting Report",
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "kor_statcast.csv"
...
def main():
    st.set_page_config(
        page_title="South Korea Scouting — WBC 2026",
    ...
    _name_ja_map = {p["name"]: p.get("name_ja", "") for p in KOR_BATTERS}
    ...
    player_names = [t["team_overview"]] + [p["name"] for p in KOR_BATTERS]
    ...
    if selected == t["team_overview"]:
        st.header(f"\U0001F1F0\U0001F1F7 {t['team_overview']}")
    ...
    st.header(f"\U0001F1F0\U0001F1F7 {_display_name(player['name'])}")

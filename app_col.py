"""WBC 2026 Colombia Batter Scouting Dashboard."""
...
from players_col_batters import COL_BATTERS, PLAYER_BY_NAME
...
TEXTS = {
    "EN": {
        "title": "Colombia Batter Scouting Report",
        ...
        "strength_note": (
            "Colombia continues to develop as a baseball nation, with a growing pipeline of MLB-caliber talent. The lineup may feature athletic hitters with speed and gap power, capable of manufacturing runs through contact and baserunning."
        ),
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "col_statcast.csv"
...
def main():
    st.set_page_config(
        page_title="Colombia Scouting — WBC 2026",
    ...
    st.sidebar.markdown(f"# \U0001F1E8\U0001F1F4 {t['title']}")
    st.sidebar.caption(t["subtitle"])

    _name_ja_map = {p["name"]: p.get("name_ja", "") for p in COL_BATTERS}
    ...
    player_names = [t["team_overview"]] + [p["name"] for p in COL_BATTERS]
    ...
    if selected == t["team_overview"]:
        ...
        st.header(f"\U0001F1E8\U0001F1F4 {t['team_overview']} — {season_label}")

        st.info(t["overview_guide"])

        rows = []
        player_stats_list = []
        for p in COL_BATTERS:
    ...
    season_label = f"{season} Season" if lang == "EN" else f"{season}年シーズン"
    st.header(f"\U0001F1E8\U0001F1F4 {_display_name(player['name'])} — {season_label}")

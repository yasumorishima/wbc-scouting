...
def _zone_names_for_bats(bats: str, lang: str) -> dict:
    ...
    Zones 1-9 from catcher's POV (coordinate-based):
...
def generate_pitching_plan(pdf: pd.DataFrame, stats: dict, player_info: dict, lang: str) -> str:
    ...
    # 2. Zone weakness analysis (3x3) — coordinate-based
    zone_names = _zone_names_for_bats(bats, lang)
    _sz_x = np.linspace(-0.83, 0.83, 4)
    _sz_z = np.linspace(1.5, 3.5, 4)
    _zv = pdf.dropna(subset=["plate_x", "plate_z"]).copy()
    zone_ba = {}
    for _row in range(3):
        for _col in range(3):
            _zn = (2 - _row) * 3 + _col + 1
            _m = ((_zv["plate_x"] >= _sz_x[_col]) & (_zv["plate_x"] < _sz_x[_col + 1])
                  & (_zv["plate_z"] >= _sz_z[_row]) & (_zv["plate_z"] < _sz_z[_row + 1]))
            _zdf = _zv[_m]
            _ab = _zdf[_zdf["events"].isin(_AB_EVENTS)]
            _hits = _zdf[_zdf["events"].isin(_HIT_EVENTS)]
            if len(_ab) >= 5:
                zone_ba[_zn] = len(_hits) / len(_ab)
...
def generate_hitting_plan(pdf: pd.DataFrame, stats: dict, pitcher_info: dict, lang: str) -> str:
    ...
    # 2. Zone vulnerability — coordinate-based (pitcher context: "R" default)
...
def main():
    ...
    with tab_gp:
        ...
        for rp_name in KEY_RELIEVERS:
            ...
            # Chase rate — coordinate-based
            _rp_chase_pct = 0.0
            _rp_coords = rp_data.dropna(subset=["plate_x", "plate_z"])
            if not _rp_coords.empty:
                _rp_outside = _rp_coords[
                    (_rp_coords["plate_x"] < -0.83) | (_rp_coords["plate_x"] > 0.83)
                    | (_rp_coords["plate_z"] < 1.5) | (_rp_coords["plate_z"] > 3.5)]

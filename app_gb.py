"""WBC 2026 Great Britain Batter Scouting Dashboard."""
...
from players_gb_batters import GB_BATTERS, PLAYER_BY_NAME
...
TEXTS = {
    "EN": {
        "title": "Great Britain Batter Scouting Report",
...
# Data loading
# ---------------------------------------------------------------------------
DATA_PATH = pathlib.Path(__file__).parent / "data" / "gb_statcast.csv"
...
def main():
    st.set_page_config(
        page_title="Great Britain Scouting — WBC 2026",
        page_icon="\U0001F1EC\U0001F1E7",
        layout="wide",
    ...
    _name_ja_map = {p["name"]: p.get("name_ja", "") for p in GB_BATTERS}
    ...
    player_names = [t["team_overview"]] + [p["name"] for p in GB_BATTERS]
    ...
    if selected == t["team_overview"]:
        ...
        st.header(f"\U0001F1EC\U0001F1E7 {t['team_overview']} — {season_label}")
    ...
    season_label = f"{season} Season" if lang == "EN" else f"{season}年シーズン"
    st.header(f"\U0001F1EC\U0001F1E7 {_display_name(player['name'])} — {season_label}")
